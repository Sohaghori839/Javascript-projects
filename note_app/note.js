...
closeIcon.addEventListener("click", () => {
    ...
    addTitle.innerText = "Add a new Note";
...
function showMenu(elem) {
    ...
    document.addEventListener("click", e => {
        ...
        if (e.target != "I" || e.target != elem) {
            elem.parentElement.classList.remove("show");
        }
    });
}
function deleteNote(noteId){
    let confirm = confirm("Are you sure you went to delete this note?")
...
function updateNote(noteId, title, desc) {
    ...
    addTitle.innerText = "Update Note";
...
addBtn.addEventListener("click", e => {
    ...
    if (noteTitle || noteDesc) {
        ...
        if (!isUpdate) {
            notes.push(noteInfo);//adding new note a notes
        }
        else{
            isUpdate = false;
            notes[updateId] = noteInfo; //updating spacified note
        }
